...
void level::interpolate_object_positions(float interpolation_ratio)
{
    ...
    for (o = first_active; o;)
    {
        ...
        if (!(dev & SUSPEND_MODE) || c)
        {
            ...
            if (c)
            {
                ...
                o->move(c->x_suggestion, c->y_suggestion,
                        c->b1_suggestion | (c->b2_suggestion << 1) | (c->b3_suggestion << 2));
                    ...
                    if (se)
                    {
                        ...
                        else
                        {
                            ...
                            for (; f; f = f->next)
                            {
                                ...
                                {
                                    if (f->otype != 0xffff &&
                                        f->current_frame >= figures[f->otype]->get_sequence(f->state)->length())
                                        f->current_frame = 0;
                                }
                            }
                        }
                ...
                for (; j < t; j++)
                {
                    ...
                    if (new_type != 0xffff) // make sure old object still exists
                    {
                        ...
                        for (; k < figures[new_type]->tiv; k++)
                        {
                            ...
                            {
                                if (!strcmp(lstring_value(((LSymbol *)figures[new_type]->vars[k])->GetName()),
                                            old_name))
