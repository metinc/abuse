...
bool AR_SPEC::AR_ConvertSPEC(std::string file_path)
{
	...
	*/
    ...
    if (pal_custom)
        this->tx_info.Write("color_palette=" + color_palette);

    if (this->pal_shift > 0)
    {
        if (pal)
            pal->shift(this->pal_shift);
        if (pal_hires)
            pal_hires->shift(this->pal_shift);
        if (pal_custom)
            pal_custom->shift(this->pal_shift);
    ...
    for (int i = 0; i < dir.total; i++)
    {
        ...
        if (se->type == SPEC_FORETILE || se->type == SPEC_BACKTILE || se->type == SPEC_IMAGE ||
            se->type == SPEC_CHARACTER || se->type == SPEC_CHARACTER2)
        {
            ...
            if (found != std::string::npos)
                name.erase(found, 4); //some images have "_hires" after ".pcx", so we just remove ".pcx"
            im->ar_name = name;
            ...
            if (se->type == SPEC_FORETILE || se->type == SPEC_BACKTILE)
            {
                if (!this->pong_the_bong)
                {
                    if (this->output_tilemap == AR_OCV_FILEPERPCX)
                    {
                        ...
                        groups[0].images.push_back(im); //put all in the same group
            ...
            else if (se->type == SPEC_IMAGE || se->type == SPEC_CHARACTER || se->type == SPEC_CHARACTER2)
            {
                ...
                else if (this->output_animation == AR_OCV_FILEPERGROUP || this->output_animation == AR_OCV_FILEPERSPEC)
                {
                    if (this->group_max > 0)
                    {
                        ...
                        {
                            if (groups[index].images.empty())
                                groups[index].name = name; //for all in one group
                    ...
                    else
                    {
                        ...
                        if (found != std::string::npos)
                        {
                            ...
                            if (group_index == -1)
                            {
                                ...
                                groups[group_index].images.push_back(im);
                        }
                        else
                        {
    ...
    if (in_tiles)
    {
        ...
        if (this->output_tilemap == AR_OCV_FILEPERSPEC)
        {
            ...
            for (int i = 0; i < tile_r; i++) //row
            {
                ...
                for (int j = 0; j < tile_c; j++) //column
                {
                    ...
                    if (!this->tilemap_palette.empty())
                    {
                        ...
                        std::string ps;
                        if (palette_stream >> ps)
                            im = AR_GetImageByName(ps, groups);
                        //else error;//TODO
                    }
                    else if (k < images.size())
                        im = images[k];

                    if (im)
                    {
                        bool pixel_alpha = true;
                        if (im->ar_type == SPEC_BACKTILE)
                            pixel_alpha = false;
            ...
            if (!AR_CreateImage(matrix, width_fin, height_fin, file_path.substr(0, file_path.size() - 4), pal,
            ...
            if (pal_custom)
                stream << "out=" << file_path.substr(0, file_path.size() - 4) << "_" + AR_GetFileName(color_palette)
                       << "." << this->image_format << std::endl;
            else
                stream << "out=" << file_path.substr(0, file_path.size() - 4) << "." << this->image_format << std::endl;
    ...
    else
    {
        ...
        if (this->output_animation == AR_OCV_FILEPERGROUP || this->output_animation == AR_OCV_FILEPERSPEC)
        {
            ...
            if (this->output_animation == AR_OCV_FILEPERSPEC)
            {
                if (pal_custom)
                    stream << "out="
                           << file_path.substr(0, file_path.size() - 4) + "_" + AR_GetFileName(color_palette) + "." +
                                  this->image_format
                           << std::endl;
                else
                    stream << "out=" << file_path.substr(0, file_path.size() - 4) + "." + this->image_format
                           << std::endl;
            ...
            for (unsigned int i = 0; i < groups.size(); i++)
            {
                if (groups[i].images.empty())
                    continue; //default all in one group
                ...
                if (this->output_animation == AR_OCV_FILEPERGROUP)
                {
                    if (pal_custom)
                        fpg << "out="
                            << file_path.substr(0, file_path.size() - 4) + "_" + groups[i].name + "_" +
                                   AR_GetFileName(color_palette) + "." + this->image_format
                            << std::endl;
                    else
                        fpg << "out="
                            << file_path.substr(0, file_path.size() - 4) + "_" + groups[i].name + "." +
                                   this->image_format
                            << std::endl;
                }
                ...
                else if (this->output_animation == AR_OCV_FILEPERSPEC)
                {
                    ...
                    if (result)
                    {
                        ...
                        //final size of output image
                        if (result->x + result->w > width_fin)
                            width_fin = result->x + result->w;
                        if (result->y + result->h > height_fin)
                            height_fin = result->y + result->h;
...
void AR_SPEC::AR_PrepareGroups(std::vector<AR_ImageGroup> &groups, int &surface, int &max_width)
{
    for (unsigned int i = 0; i < groups.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < images.size(); j++)
        {
            ...
            if (j < images.size() - 1)
                width += this->tilemap_padding; //add gap/padding between two frames
        }
        ...
        if (groups[i].w > max_width)
            max_width = groups[i].w;
    }
                             ...
                             palette *pal_hires, palette *pal_custom)
{
    ...
    for (int i = 0; i < ocv.rows; i++)
    {
        ...
        for (int j = 0; j < ocv.cols; j++)
        {
            ...
            if (pal_custom)
                pal_fin = pal_custom; //use preloaded pallete
            else
            {
