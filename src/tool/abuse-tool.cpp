...
int abuse_tool(int argc, char *argv[])
{
    ...
    if (cmd == CMD_LIST)
    {
        ...
        for (int i = 0; i < dir.total; i++)
        {
            ...
            char buffer[512];
            sprintf(buffer, "% 5i   % 3i % 8i  %04x  %s", i, se->type, (int)se->size, calc_crc(se->data, se->size),
                    se->name);
            row << buffer;
    ...
    else if (cmd == CMD_PUT || cmd == CMD_PUTPCX)
    {
        ...
        dir.FullyLoad(&fp);
        dir.total++;
        dir.entries = (spec_entry **)realloc(dir.entries, dir.total * sizeof(spec_entry *));
