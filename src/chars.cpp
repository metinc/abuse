...
char const *state_names[MAX_STATE] = {"dead",        "dieing",        "stopped",      "start_run_jump",
                                      "run_jump",    "run_jump_fall", "end_run_jump", "flinch_up",
                                      "flinch_down", "morph_pose",    "running"
#if 0
...
char const *ofun_names[TOTAL_OFUNS] = {"ai_fun",     "move_fun",           "draw_fun",       "map_draw_fun",
                                       ...
                                       "reload_fun", "get_cache_list_fun", "type_change_fun"};
...
void l_obj_set(long number, void *arg) // exten lisp function switches on number
{
    ...
    if (t->tiv <= number || !t->vars[number])
    {
        lbreak("set : variable does not exist for this class\n");
        return;
    }
    ...
    for (; field; field = CDR(field))
    {
        ...
        if (f == l_abil)
        {
            ...
            for (int i = 0; i < TOTAL_ABILITIES; i++)
            {
                ...
                PtrRef r5(ab);
                if (!NILP(ab))
                    abil[i] = lnumber_value(leval(lcar(lcdr(ab))));
            }
        }
        else if (f == l_funs)
        {
        ...
        {
            rangex = lnumber_value(leval(lcar(lcdr(lcar(field)))));
            rangey = lnumber_value(leval(lcar(lcdr(lcdr(lcar(field))))));
        }
        else if (f == l_draw_range)
        {
            draw_rangex = lnumber_value(leval(lcar(lcdr(lcar(field)))));
            draw_rangey = lnumber_value(leval(lcar(lcdr(lcdr(lcar(field))))));
        }
        else if (f == l_states)
        {
            ...
            PtrRef r4(l);
            char fn[100];
            strcpy(fn, lstring_value(leval(CAR(l))));
            l = CDR(l);
            while (l)
            {
                int index;
        ...
        else if (f == l_fields)
        {
            ...
            while (!NILP(mf))
            {
                char *real = lstring_value(leval(lcar(lcar(mf))));
                char *fake = lstring_value(leval(lcar(lcdr(lcar(mf)))));
        ...
        {
            char *fn = lstring_value(leval(CAR(CDR(CAR(field)))));
            char *o = lstring_value(leval(CAR(CDR(CDR(CAR(field))))));
