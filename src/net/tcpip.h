...
class ip_address : public net_address
{
  public:
    ...
    virtual protocol protocol_type() const
    {
        return net_address::IP;
    }
...
class tcpip_protocol : public net_protocol
{
  ...
  public:
    ...
    net_socket *connect_to_server(net_address *addr, net_socket::socket_type sock_type = net_socket::SOCKET_SECURE);
...
class udp_socket : public unix_fd
{
  public:
    ...
    {
        if (addr)
            return sendto(fd, (char *)buf, size, 0, (sockaddr *)(&((ip_address *)addr)->addr),
