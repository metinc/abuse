            ...
            if (comm_sock && comm_sock->ready_to_read()) // new connection?
            {
                ...
                if (new_sock)
                {
                    ...
                    else
                    {
                        ...
                        break;
                        default: {
                            if (game_face->add_client(client_type, new_sock, addr) ==
                                0) // ask server or client to add new client
                            {
                                delete addr;
                                delete new_sock;
                            }
...
int request_server_entry()
{
    if (prot && main_net_cfg)
    {
        ...
        if (sock->write(&len, 1) != 1 || sock->write(uname, len) != len ||
...
int become_server(char *name)
{
    if (prot && main_net_cfg)
    {
        ...
        if (game_sock)
            delete game_sock;
        game_sock = prot->create_listen_socket(
            main_net_cfg->port + 1, net_socket::SOCKET_FAST); // this is used for fast game packet transmission
        if (!game_sock)
        {
            if (comm_sock)
                delete comm_sock;
