...
#define p_dist(x1, y1, x2, y2)                                                                                         \
    (((int)(x1) - (int)x2) * ((int)(x1) - (int)x2) + ((int)(y1) - (int)y2) * ((int)(y1) - (int)y2))

super_morph::super_morph(TransImage *hint1, TransImage *hint2, int aneal_steps, void (*stat_fun)(int))
{
    int x, y, w1 = hint1->Size().x, h1 = hint1->Size().y, w2 = hint2->Size().x, h2 = hint2->Size().y;
    if (w1 > w2)
        w = w1;
    else
        w = w2;
    ...
    unsigned short start1[256], start2[256], totals[256], start = 0, *h1p = hints1, *h2p = hints2;
...
int smorph_player::show(image *screen, int x, int y, ColorFilter *fil, palette *pal, int blur_threshold)
{
    if (!f_left)
        return 0;
    ...
    for (ss = steps, i = 0; i < t; i++, ss++)
    {
        ...
        {
            hole[ix + iy * w] = *(screen->scan_line(py) + px) =
                fil->Lookup(ss->r >> (19), ss->g >> (19), ss->b >> (19));
        }
        ss->x += ss->dx;
        ss->y += ss->dy;
        ss->r += ss->dr;
      ...
      if (*nl) t++; */
                ...
                if (*tl)
                {
                    if (t)
                    {
                        ...
                        g /= t;
                        b /= t;
                        int dist = ((int)(*pa) - r) * ((int)(*pa) - r);
                        pa++;
                        dist += ((int)(*pa) - g) * ((int)(*pa) - g);
                        pa++;
                    ...
                    *(tl) = *(screen->scan_line(y + iy) + x + ix) =
                        fil->Lookup((r / t) >> 3, (g / t) >> 3, (b / t) >> 3);
            }
        }
        ll += 2;
        tl += 2;
