...
char const *spec_types[] = {"Invalid type", // 0
                            ...
                            "Invalid Type", // 3
                            "Image", // 4
                            "Fore Tile",      "Back Tile",    "Character",  "8 Morph",       "16 Morph",
                            "Grue objs",      "Extern WAV",   "DMX MUS",    "Patched morph", "Normal file",
                            "Compress1 file", "Vector Image", "Light list", "Grue fgmap",    "Grue bgmap",
                            "Data array",     "Character2",   "Particle",   "Extern lcache"};
...
class null_file : public bFILE // this file type will use virtual opens inside of a spe
{
  public:
    ...
    virtual int unbuffered_read(void *buf, size_t count)
    {
        return 0;
    }
    virtual int unbuffered_write(void const *buf, size_t count)
    {
        return 0;
    }
...
spec_entry::spec_entry(uint8_t spec_type, char const *object_name, char const *link_filename, unsigned long data_size,
...
{
    int i;
    for (i = 0; i < total && entries[i] != e; i++)
        ; // find the entry in the array first
