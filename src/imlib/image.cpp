...
image_descriptor::image_descriptor(ivec2 size, int keep_dirties, int static_memory)
{
    m_aa = ivec2(0);
    m_bb = size;
    m_size = size;
...
{
    CONDITION(pos.x >= 0 && pos.x < m_size.x && pos.y >= 0 && pos.y < m_size.y, "image::Pixel Bad pixel xy");
...
image::image(ivec2 size, uint8_t *page_buffer, int create_descriptor)
{
    ...
    if (create_descriptor || page_buffer)
        m_special = new image_descriptor(size, create_descriptor == 2, (page_buffer != NULL));
    ...
    if (m_special)
    {
        if (m_special->x1_clip() < m_special->x2_clip())
            for (int j = m_special->y1_clip(); j < m_special->y2_clip(); j++)
                memset(scan_line(j) + m_special->x1_clip(), color, m_special->x2_clip() - m_special->x1_clip());
    }
    else
...
void image_descriptor::DeleteDirty(ivec2 aa, ivec2 bb)
{
    ...
    for (p = (dirty_rect *)dirties.first(); i; i--, p = next)
    {
        ...
        else
        {
            ...
            if (bb.x - 1 >= p->m_bb.x || aa.x <= p->m_aa.x)
            {
                ax1 = p->m_aa.x;
                ax2 = p->m_bb.x + 1;
            }
...
{
    int16_t x, y, j;
    uint8_t dt_matrix[] = {0, 136, 24, 170, 68, 204, 102, 238, 51, 187, 17, 153, 119, 255, 85, 221};
        ...
        for (j = 0; j < m_size.y; j++)
        {
            for (t = 0, k = -smoothness; k <= smoothness; k++)
                for (l = -smoothness; l <= smoothness; l++)
                    if (i + k > smoothness && i + k < m_size.x - smoothness && j + l < m_size.y - smoothness &&
                        j + l > smoothness)
...
void image::WidgetBar(ivec2 p1, ivec2 p2, uint8_t light, uint8_t med, uint8_t dark)
{
...
uint8_t dither_matrix[] = {0, 136, 24, 170, 68, 204, 102, 238, 51, 187, 17, 153, 119, 255, 85, 221};
