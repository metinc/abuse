...
int palette::find_closest(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for (i = 0; i < 256; i++)
    {
        nd = ((int)r - (int)(*cl)) * ((int)r - (int)(*cl));
        cl++;
        nd += ((int)g - (int)(*cl)) * ((int)g - (int)(*cl));
        cl++;
        nd += ((int)b - (int)(*cl)) * ((int)b - (int)(*cl));
        cl++;
        if (nd < d)
        {
            c = i;
...
quant_node::quant_node(int level, quant_node *dad, unsigned char r, unsigned char g, unsigned char b)
{
    int i;
