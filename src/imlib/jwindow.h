...
class Jwindow
{
  ...
  public:
    ...
    {
        m_surf->Bar(ivec2(x1(), y1()), ivec2(x2(), y2()), color);
    }
    void show()
    {
        _hidden = false;
    ...
    void clip_in()
    {
        m_surf->SetClip(ivec2(x1(), y1()), ivec2(x2() + 1, y2() + 1));
    }
    void clip_out()
    {
        m_surf->SetClip(ivec2(0), m_size);
    }
    char *read(int id)
    {
        return inm->get(id)->read();
    }
    void local_close();
...
class WindowManager : public EventHandler
{
  ...
  public:
    ...
    Jwindow *CreateWindow(ivec2 pos, ivec2 size, ifield *fields = NULL, char const *Name = NULL);
