...
class text_field : public ifield
{
    ...
    int xend()
    {
        return m_pos.x + wm->font()->Size().x * (strlen(prompt) + 1 + strlen(format)) + 7;
    }
    int yend()
    {
        return m_pos.y + wm->font()->Size().y + 5;
    }
  ...
  public:
    ...
    text_field(int X, int Y, int ID, char const *Prompt, char const *Format, double Data, ifield *Next);
