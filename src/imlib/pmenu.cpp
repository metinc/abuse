...
pmenu::pmenu(int X, int Y, pmenu_item *first, image *screen)
{
    ...
    bar->m_surf->WidgetBar(ivec2(0, 0), ivec2(w - 1, h - 1), wm->bright_color(), wm->medium_color(), wm->dark_color());
...
void psub_menu::draw(Jwindow *parent, int x, int y)
{
    ...
    win = wm->CreateWindow(parent->m_pos + ivec2(x, y),
                           ...
                           NULL);
    win->freeze();
    win->m_surf->WidgetBar(ivec2(0, 0), ivec2(w - 1, h - 1), wm->bright_color(), wm->medium_color(), wm->dark_color());
...
void pmenu_item::draw_self(Jwindow *parent, int x, int y, int w, int top, int active)
{
    ...
    if (!n)
    {
        ...
        parent->m_surf->WidgetBar(ivec2(x, y + h / 2 - 1), ivec2(x + w - 1, y + h / 2), wm->dark_color(),
    ...
    else
    {
        if (active)
        {
            ...
            {
                parent->m_surf->Bar(ivec2(bx, y), ivec2(x + w - 1, y + wm->font()->Size().y + 1), wm->dark_color());
                wm->font()->PutString(parent->m_surf, ivec2(x + 1, y + 1), n, wm->medium_color());
                if (on_off && *on_off)
                    wm->font()->PutString(parent->m_surf, ivec2(bx + 1, y + 1), "*", wm->medium_color());
            }
        }
        else
        {
            ...
            {
                parent->m_surf->Bar(ivec2(bx, y), ivec2(x + w - 1, y + wm->font()->Size().y + 1), wm->medium_color());
                wm->font()->PutString(parent->m_surf, ivec2(x + 1, y + 1), n, wm->bright_color());
                if (on_off && *on_off)
                    wm->font()->PutString(parent->m_surf, ivec2(bx + 1, y + 1), "*", wm->bright_color());
            }
        }
    }
}
...
int psub_menu::handle_event(Jwindow *parent, int x, int y, Event &ev)
{
    ...
    if (ev.mouse_move.x >= x && ev.mouse_move.y >= y && ev.mouse_move.x < x + w && ev.mouse_move.y < y + h)
    {
        ...
        return item_num(active)->handle_event(win, win->m_pos.x + dx, win->m_pos.y + 3 + active * (th + 1), w - 6, 0,
                                              ev);
    else
        return 0;
}
...
int pmenu_item::handle_event(Jwindow *parent, int x, int y, int w, int top, Event &ev)
{
    ...
    else if (sub)
    {
        if (top)
            ...
            return sub->handle_event(parent, x + w, y, ev);
    }
    else
        return 0;
}
...
pmenu_item *pmenu::inarea(int mx, int my, image *screen)
{
    mx -= bar->m_pos.x;
    my -= bar->m_pos.y;
    if (mx < 0 || my < 0 || mx >= bar->m_surf->Size().x || my >= bar->m_surf->Size().y)
        return NULL;
    else
    {
...
int pmenu::handle_event(Event &ev, image *screen)
{
    ...
    case EV_MOUSE_MOVE: {
        ...
        if (!new_selection && active && active->handle_event(bar, itemx(active), 1, itemw(active), 1, ev))
            return 1;
