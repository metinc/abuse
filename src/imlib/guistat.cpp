...
void gui_status_manager::draw_bar(gui_status_node *whom, int perc)
{
    ...
    whom->stat_win->m_surf->Bar(ivec2(whom->stat_win->x1() + 1, whom->stat_win->y2() - h - 1),
                                ivec2(whom->stat_win->x2() - 1, whom->stat_win->y2() - 1), wm->black());
    whom->stat_win->m_surf->Bar(ivec2(whom->stat_win->x1() + 2, whom->stat_win->y2() - h),
                                ivec2(whom->stat_win->x2() - 2, whom->stat_win->y2() - 2), wm->dark_color());
    if (perc)
        whom->stat_win->m_surf->Bar(ivec2(whom->stat_win->x1() + 3, whom->stat_win->y2() - h + 1),
                                    ivec2(whom->stat_win->x1() + l * perc / 100, whom->stat_win->y2() - 3),
                                    wm->bright_color());
}
    ...
    if (first)
    {
        if (!first->stat_win)
        {
            ...
            if (now.diff_time(&first->last_time) > 1)
            {
                ...
                if (first->show)
                    first->show->draw(first->stat_win->m_surf,
                                      (first->stat_win->x2() - first->stat_win->x1()) / 2 - first->show->width() / 2,
                                      my + h1, NULL);
