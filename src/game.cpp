                ...
                for (x = x1, draw_x = xo; x <= x2; x++, draw_x += xinc, cl++)
                {
                    if (above_tile(*cl))
                    {
                        ...
                        if (fort_num != BLACK)
                        {
                            ...
                            {
                                main_screen->Line(ivec2(draw_x, draw_y), ivec2(draw_x + xinc, draw_y + yinc),
                                                  wm->bright_color());
                                main_screen->Line(ivec2(draw_x + xinc, draw_y), ivec2(draw_x, draw_y + yinc),
                                                  wm->bright_color());
                            }
                        }
                    }
...
void Game::update_screen(uint32_t elapsedMsFixed)
{
    ...
    else if (current_level)
    {
        ...
        {
            for (view *f = first_view; f; f = f->next)
                main_screen->PutImage(
                    cache.img(pause_image),
                    ivec2((f->m_aa.x + f->m_bb.x) / 2 - cache.img(pause_image)->Size().x / 2, f->m_aa.y + 5), 1);
        }
...
void Game::get_input()
{
    ...
    while (event_waiting())
    {
        ...
        if (ev.type != EV_KEY || !key_down(ev.key) || ev.window || (dev & EDIT_MODE))
        {
            ...
            case PAUSE_STATE: {
                if (ev.type == EV_KEY &&
                    (ev.key == 'p' || ev.key == JK_SPACE || ev.key == JK_ENTER || ev.key == JK_ESC))
                {
                    set_state(RUN_STATE);
                }
            }
