...
void show_general_settings()
{
    while (true)
    {
        ...
        choice_picker *skin = new choice_picker(content_x, skin_picker_y, ID_PLAYER_SKIN_PICKER, skin_labels,
                                                settings.player_skin, nullptr);
        int skin_x1, skin_y1, skin_x2, skin_y2;
        skin->area(skin_x1, skin_y1, skin_x2, skin_y2);
        const int skin_control_width = skin_x2 - skin_x1 + 1;
        ...
        const int content_bottom = std::max(skin_y2 + 1, skin_picker_y + skin_preview_height) + padding;
