...
Cheats are available in single-player games and in multiplayer games when using a Debug build. Release builds reject multiplayer cheat commands. To use cheats, press <kbd>Enter</kbd> to open the console and type the desired slash-prefixed cheat command. The mouse cursor must be inside the console window for input. Press Enter when done, or type "quit"/"exit" to close the console. Use the scrollbar, mouse wheel, or arrow keys to browse the chat history.
